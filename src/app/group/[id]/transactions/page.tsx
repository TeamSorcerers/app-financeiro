...
import { PAD_LENGTH, ZERO } from "@/lib/shared/constants";
...
export default function GroupTransactionsPage ({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  const loadGroupData = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(body?.error || "Erro ao carregar transações do grupo");
      ...
      setGroupName(json.group?.name || "Grupo");
      setTransactions(json.transactions || []);

      // Verificar se é admin
      const currentMember = json.members?.find((m: { userId: number }) => m.userId === session?.user?.userId);

      setIsAdmin(currentMember?.isOwner || false);
    } catch (e: unknown) {
  ...
  }, [ sessionStatus, groupId, loadGroupData ]);
  ...
  return (
    <div className="min-h-screen bg-[#F0F0F3] p-4 sm:p-6">
      ...
      <header className="mb-6">
        ...
        >
          <ArrowLeft size={20} />
          <span className="text-sm font-medium">Voltar para o grupo</span>
        </button>

        <h1 className="text-2xl sm:text-3xl font-bold text-[#4A90E2] mb-2">
          Transações - {groupName}
        </h1>
        <p className="text-sm text-[#6a6a6a]">Histórico completo de receitas e despesas do grupo</p>
      </header>
